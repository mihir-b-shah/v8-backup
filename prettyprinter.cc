...
void AstPrinter::VisitAssignment(Assignment* node) {
  std::cout << "Assignment node: " << node << " LHS: " << node->target() << " RHS: " << node->value();
  if(node->target()->IsVariableProxy()){
    std::cout << " INIT: " << static_cast<VariableProxy*>(node->target())->var()->initializer();
  }
  std::cout << '\n';
