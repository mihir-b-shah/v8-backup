...
IGNITION_HANDLER(SwitchOnSmiNoFeedback, InterpreterAssembler) {
  ...
  */
  Label fall_through(this), convert(this), low_check(this), high_check(this), is_not_smi(this);
  ...
  BIND(&low_check);
  BranchIfNumberGreaterThanOrEqual(CAST(acc), SmiConstant(Smi::kMinValue), &high_check, &fall_through);

  BIND(&high_check);
  BranchIfNumberLessThanOrEqual(CAST(acc), SmiConstant(Smi::kMaxValue), &convert, &fall_through);

  //Branch(BoolConstant(TryToSmiConstant(ReinterpretCast<IntegralT>(acc), &cv)), &convert, &fall_through);
